...
export default function configureNodeHandlers(apiRouter: Router) {
  ...
  apiRouter.delete(
    '/node/logs',
    doubleCsrfProtection,
    asyncRouteHandler(async (req: Request, res: Response<NodeClearLogsResponse>) => {
  ...
  apiRouter.get(
    ...
    asyncRouteHandler(async (req: Request, res: Response<NodeStatusResponse>) => {
      ...
      const output = execFileSync('/usr/local/bin/operator-cli', ['is_genesis_node', address], {
        encoding: 'utf8',
      })
  ...
  apiRouter.get(
    ...
    asyncRouteHandler(async (req: Request, res: Response<NodeNetworkResponse>) => {
      ...
      const output = execFileSync('/usr/local/bin/operator-cli', ['network-stats'], {
        encoding: 'utf8',
      })
      const yamlData = yaml.load(output)
      res.json(yamlData)
