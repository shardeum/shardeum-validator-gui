...
export const SettingsDisplay = () => {
  ...
  useEffect(() => {
    setStopDirectly(nodeStatus?.state === 'standby' || nodeStatus?.state === 'need-stake')
  }, [nodeStatus?.state])
    ...
    <div className="grow flex flex-col justify-between">
      <div className="md:px-16 px-4 flex flex-col gap-y-12">
        <div className="flex flex-col gap-y-2">
          ...
          <span className="text-sm bodyFg">Change settings and configure your node from this panel.</span>
        </div>
        <AutoRestartNodeToggle isOn={settings?.autoRestart || false} onClick={toggleAutoRestart} />
        {nodeStatus && (
          <ForceRemoveStake
            isEnabled={isConnected && chain?.id === CHAIN_ID && (stakeInfo?.stake ?? '0.0') > '0.0'}
        ...
        {nodeStatus && <ForceStopNode isEnabled={nodeStatus.state !== 'stopped'} needsConfirmation={!stopDirectly} />}
        <PasswordResetForm />
      </div>
